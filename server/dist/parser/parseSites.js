...
async function parseEmall(page = 1) {
...
async function parse21vek(page = 1) {
...
async function parseDetmir(offset = 0) {
    ...
    const preParseData = items.map((item) => {
        return {
            title: item.title,
...
async function parseYamigom() {
    ...
    const items = (await response.json());
    const preParseData = items[0].offers.map((item) => {
        return {
            title: item.name,
    ...
    console.log('Yamigom: ', preParseData.length);
    return preParseData;
}
const parseFuncArr = [parseEmall, parse21vek, parseDetmir, parseYamigom];
